...
public class healthSector {
        ...
        try {
            ...
            for (FogDevice device : fogDevices) {
                if (device.getName().startsWith("edge")) {
                    moduleMapping.addModuleToDevice("biometric_module", device.getName());
                    moduleMapping.addModuleToDevice("pir_module", device.getName());
                    moduleMapping.addModuleToDevice("bloodOxygen_module", device.getName());
            ...
            if (CLOUD) {
                moduleMapping.addModuleToDevice("biometric_module", "cloud");
                moduleMapping.addModuleToDevice("pir_module", "cloud");
                moduleMapping.addModuleToDevice("bloodOxygen_module", "cloud");
    ...
    private static void linkSensorsAndActuatorsToFogDevices(int userId, String appId, FogDevice edgeDevice) {
        Sensor biometricSensor = new Sensor("biometricSensor-" + edgeDevice.getName(), "BIOMETRIC", userId, appId, new DeterministicDistribution(5)); // Generates data every 5 seconds
        biometricSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(biometricSensor);

        Sensor pirSensor = new Sensor("pirSensor-" + edgeDevice.getName(), "PIR", userId, appId, new DeterministicDistribution(6)); // Generates data every 6 seconds
        pirSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(pirSensor);

        Sensor bloodOxygenSensor = new Sensor("bloodOxygenSensor-" + edgeDevice.getName(), "BLOOD_OXYGEN", userId, appId, new DeterministicDistribution(4)); // Generates data every 4 seconds
        bloodOxygenSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(bloodOxygenSensor);
    ...
    private static Application createApplication(String appId, int userId) {
        ...
        application.addAppModule("biometric_module", 20); // Processes biometric data
        application.addAppModule("pir_module", 20); // Processes PIR data
        application.addAppModule("bloodOxygen_module", 20); // Processes blood oxygen data
        ...
        application.addAppEdge("BIOMETRIC", "biometric_module", 1000, 2000, "BIOMETRIC", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("PIR", "pir_module", 1000, 500, "PIR", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("BLOOD_OXYGEN", "bloodOxygen_module", 1000, 500, "BLOOD_OXYGEN", Tuple.UP, AppEdge.SENSOR);
        ...
        application.addAppEdge("biometric_module", "data_analyzer", 500, 100, "BIOMETRIC_ALERT", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("pir_module", "data_analyzer", 500, 100, "PIR_ALERT", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("bloodOxygen_module", "data_analyzer", 500, 100, "BLOOD_OXYGEN_ALERT", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("ecg_module", "data_analyzer", 500, 100, "ECG_ALERT", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("temperature_module", "data_analyzer", 500, 100, "TEMPERATURE_ALERT", Tuple.UP, AppEdge.MODULE);
        ...
        application.addTupleMapping("biometric_module", "BIOMETRIC", "BIOMETRIC_ALERT", new FractionalSelectivity(0.7));
        application.addTupleMapping("pir_module", "PIR", "PIR_ALERT", new FractionalSelectivity(1.0));
        application.addTupleMapping("bloodOxygen_module", "BLOOD_OXYGEN", "BLOOD_OXYGEN_ALERT", new FractionalSelectivity(0.7));
        application.addTupleMapping("ecg_module", "ECG", "ECG_ALERT", new FractionalSelectivity(0.5));
        application.addTupleMapping("temperature_module", "TEMPERATURE", "TEMPERATURE_ALERT", new FractionalSelectivity(0.3));

        // Define loops to monitor latency-sensitive operations
        final AppLoop loop1 = new AppLoop(new ArrayList<String>() {{
            add("biometric_module");
            add("data_analyzer");
        }});
