...
public class AgricultureFarmingSector {

    static List<FogDevice> fogDevices = new ArrayList<>();
    static List<Sensor> sensors = new ArrayList<>();
    static List<Actuator> actuators = new ArrayList<>();
    static int numOfZones = 7;
    ...
    public static void main(String[] args) {
        try {
            ...
            String appId = "agricultureMonitoring";
            ...
            Log.printLine("Agriculture and Farming Monitoring Simulation finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void linkSensorsAndActuatorsToFogDevices(int userId, String appId, FogDevice edgeDevice) {
        Sensor soilMoistureSensor = new Sensor("soilMoistureSensor-" + edgeDevice.getName(), "SOIL_MOISTURE", userId, appId, new DeterministicDistribution(5)); // Generates data every 5 seconds
        soilMoistureSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(soilMoistureSensor);

        Sensor airQualitySensor = new Sensor("airQualitySensor-" + edgeDevice.getName(), "AIR_QUALITY", userId, appId, new DeterministicDistribution(6)); // Generates data every 6 seconds
        airQualitySensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(airQualitySensor);

        Sensor temperatureSensor = new Sensor("temperatureSensor-" + edgeDevice.getName(), "TEMPERATURE", userId, appId, new DeterministicDistribution(4)); // Generates data every 4 seconds
        temperatureSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(temperatureSensor);

        Sensor humiditySensor = new Sensor("humiditySensor-" + edgeDevice.getName(), "HUMIDITY", userId, appId, new DeterministicDistribution(6)); // Generates data every 6 seconds
        humiditySensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(humiditySensor);

        Sensor lightSensor = new Sensor("lightSensor-" + edgeDevice.getName(), "LIGHT", userId, appId, new DeterministicDistribution(5)); // Generates data every 5 seconds
    ...
    private static Application createApplication(String appId, int userId) {
        ...
        application.addAppModule("soil_moisture_module", 20); // Processes soil moisture data
        ...
        application.addAppEdge("soil_moisture_module", "data_analyzer", 500, 100, "SOIL_MOISTURE_ALERT", Tuple.UP, AppEdge.MODULE);
        ...
        application.addTupleMapping("soil_moisture_module", "SOIL_MOISTURE", "SOIL_MOISTURE_ALERT", new FractionalSelectivity(0.7));
        application.addTupleMapping("air_quality_module", "AIR_QUALITY", "AIR_QUALITY_ALERT", new FractionalSelectivity(1.0));
        application.addTupleMapping("temperature_module", "TEMPERATURE", "TEMPERATURE_ALERT", new FractionalSelectivity(0.7));
        application.addTupleMapping("humidity_module", "HUMIDITY", "HUMIDITY_ALERT", new FractionalSelectivity(0.5));
        application.addTupleMapping("light_module", "LIGHT", "LIGHT_ALERT", new FractionalSelectivity(0.3));

        // Define loops to monitor latency-sensitive operations
        final AppLoop loop1 = new AppLoop(new ArrayList<String>() {{
            add("soil_moisture_module");
            add("data_analyzer");
        }});
