...
public class Wearables {
        ...
        try {
            ...
            for (FogDevice device : fogDevices) {
                if (device.getName().startsWith("edge")) {
                    ...
                    moduleMapping.addModuleToDevice("gyro_module", device.getName());
                }
            }
            if (CLOUD) {
                ...
                moduleMapping.addModuleToDevice("gyro_module", "cloud");
    ...
    private static void linkSensorsAndActuatorsToFogDevices(int userId, String appId, FogDevice edgeDevice) {
        ...
        Sensor tempSensor = new Sensor("tempSensor-" + edgeDevice.getName(), "TEMPERATURE", userId, appId, new DeterministicDistribution(4)); // Generates data every 4 seconds
        tempSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(tempSensor);

        Sensor gpsSensor = new Sensor("gpsSensor-" + edgeDevice.getName(), "GPS", userId, appId, new DeterministicDistribution(6)); // Generates data every 6 seconds
        gpsSensor.setGatewayDeviceId(edgeDevice.getId());
        sensors.add(gpsSensor);

        Sensor gyroSensor = new Sensor("gyroSensor-" + edgeDevice.getName(), "GYROSCOPE", userId, appId, new DeterministicDistribution(5)); // Generates data every 5 seconds
        gyroSensor.setGatewayDeviceId(edgeDevice.getId());
    ...
    private static Application createApplication(String appId, int userId) {
        ...
        application.addAppModule("temp_module", 20); // Processes temperature data
        application.addAppModule("gps_module", 20); // Processes GPS data
        application.addAppModule("gyro_module", 20); // Processes gyroscope data
        ...
        application.addAppEdge("TEMPERATURE", "temp_module", 1000, 500, "TEMPERATURE", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("GPS", "gps_module", 1000, 500, "GPS", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("GYROSCOPE", "gyro_module", 1000, 500, "GYROSCOPE", Tuple.UP, AppEdge.SENSOR);
        ...
        application.addAppEdge("gyro_module", "data_analyzer", 500, 100, "GYRO_ALERT", Tuple.UP, AppEdge.MODULE);
        ...
        application.addTupleMapping("temp_module", "TEMPERATURE", "TEMP_ALERT", new FractionalSelectivity(0.7));
        application.addTupleMapping("gps_module", "GPS", "GPS_ALERT", new FractionalSelectivity(0.5));
        application.addTupleMapping("gyro_module", "GYROSCOPE", "GYRO_ALERT", new FractionalSelectivity(0.3));
